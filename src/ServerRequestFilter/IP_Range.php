...
final class Ip_Range
{
    ...
    {
        if (str_contains($ip, ':')) {
            return self::matches_i_pv6($ip, $cidr);
        }
        return self::matches_i_pv4($ip, $cidr);
    }
    /** @psalm-pure */
    public static function matches_i_pv4(string $ip, string $cidr): bool
    {
        $mask = 32;
        $subnet = $cidr;
    ...
    /** @psalm-pure */
    public static function matches_i_pv6(string $ip, string $cidr): bool
    {
        $mask = 128;
        $subnet = $cidr;
        if (str_contains($cidr, '/')) {
        ...
        $bin_mask = str_repeat('f', intval($mask / 4));
        switch ($mask % 4) {
            case 0:
                break;
            case 1:
